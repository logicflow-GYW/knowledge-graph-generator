...
* **移除了失效的并发控制：** 删除了设置面板中的“最大并发数” (Max Concurrent Requests) 选项。
    * 该功能此前处于失效状态，为了避免误导用户配置无效参数，并保持设置界面的整洁，已将其从 UI 及内部逻辑中彻底移除。

### 🧹 内部 (Internal)

* **配置清理：** 同步清理了 `settings.ts` 中残留的并发控制相关代码，精简了插件体积。
