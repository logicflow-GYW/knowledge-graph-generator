...
* **Local-First 原则：** 本插件遵循 Obsidian 的本地优先理念。您的所有笔记数据、生成的知识图谱均**100% 存储在您本地的设备上**。
* **API Key 安全：** 您的 OpenAI / Google Gemini API Key 仅加密存储在您本地 Obsidian 库的配置文件 (`.obsidian/plugins/knowledge-graph/data.json`) 中。
* **无中间人服务器：** 本插件**直接**从您的设备向 OpenAI (`api.openai.com`) 或 Google (`generativelanguage.googleapis.com`) 发起请求。**绝不会**将您的 Key 或数据发送到开发者服务器或任何第三方中转服务器。
* **零遥测 (No Telemetry)：** 本插件**不包含**任何用户行为追踪或数据收集代码。
...
* **高度可定制化**：从输出目录到每一个 AI 阶段的 Prompt，一切皆可配置。
...
## ⚠️ 费用与风险预警 (Cost & Disclaimer)

请务必注意以下事项：

1.  **API 费用消耗：** 本插件使用第三方 LLM 服务。生成的每一篇笔记、每一次 AI 审核、每一次修正都会消耗 Token 并产生费用。
2.  **关于“自动提取新概念”功能：** * 如果在设置中开启了 `Extract new concepts`，插件会自动从生成的笔记中提取 `[[链接]]` 并加入生成队列。
    * **警告：** 这是一个递归过程。如果不加监控，可能会导致生成任务指数级增长，从而产生高额 API 费用。建议初次使用时关闭此功能，或配合“队列管理仪表盘”密切监控。
3.  **免责声明：** 本插件仅作为 AI 接口的调用工具，生成的内容准确性取决于 AI 模型。作者不对因使用本插件产生的 API 费用或生成内容的准确性承担责任。
